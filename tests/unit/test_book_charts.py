...
def test_the_register_holds_every_chart_supplied_so_far():
    assert numbers() == (
        ...
        49, 50, 51, 52, 53, 54)
    # 4 is uncited; 55 and 61 are cited by sections we have read and
    # promised in sections still to come
    assert CHARTS_NOT_SUPPLIED == (4, 55, 61)
...
#: Chart 45 is a nativity whose degrees were withheld; Chart 52 is a transit
#: chart, so it has no birth data to give degrees for.
DIAGRAM_ONLY = (45, 52)
...
def test_the_index_endpoint_lists_every_chart(client):
    ...
    assert body["not_supplied"] == [4, 55, 61]
