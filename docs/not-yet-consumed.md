...
| `GATI_NAMES` | The two movements §19.4 names. `kendraadi gati` is built and is `HOUSE_ORDER`. `mandooki gati` is now **defined**: §24.3.3 supplied the Kalachakra discussion §19.4 pointed at, and `kalachakra.transitions` classifies every frog's leap — all 3rds or 11ths, as §19.4 said. Mandooka dasa itself is still unbuilt |
...
| `MANDOOKA_DASA_IS_OF_RUDRAMSA` | What the real Mandooka dasa is for — wars and death, read in D-11. The chart is in the varga registry and mandooki gati is now defined (§24.3.3); what is still missing is the dasa itself — no seed, direction or lengths anywhere in the book |
| `NAMES` | Sudasa's three names. "Rasi Dasa" unqualified means this one specifically, not the category Narayana and Lagna Kendradi also belong to — a reading layer taking a system by name needs that |
...
| `STANDARD_RESULT_TABLES` · `SEVEN_TABLES_ARE_PROMISED` | The roadmap for §25.2, now **complete** — all seven tables in. A test fails if the register and `STANDARD_RESULTS` disagree |
...
### Chapter 26 — §26.6, Tables 65 to 69 (17)

`transits/tara.py`. **All five tables have arrived.** `body_part` reads a
transit through them and `grahas_dwelling_in` runs §26.6's second purpose
backwards over the complete set.
...
| `FIVE_TABLES_COVER_NINE_BODIES_INCLUDING_THE_NODES` | Ours, and a **correction**: §26.6 says "planets" and Table 69 covers Saturn with both nodes, so it is nine bodies, not seven |
| `BODY_PART_TABLES` · `BODY_PART_TABLES_PENDING` · `TABLE_65_SUN` · `TABLE_66_MOON` · `TABLE_67_MARS` · `TABLE_68_MERCURY_JUPITER_VENUS` · `TABLE_69_SATURN_RAHU_KETU` | The registry and the Sun's table. `body_part` and `grahas_dwelling_in` read them; the register entry stays while four tables are missing |
...
| `ONLY_THE_HEAD_APPEARS_IN_EVERY_TABLE` | Ours, and the **end** of the Two feet observation: Table 69 has no such part, splitting the legs instead |
| `ROW_COUNT_DOES_NOT_TRACK_HOW_MANY_GRAHAS_SHARE_A_TABLE` | Ours, and a **correction** of what Table 68 suggested: 68 and 69 both cover three bodies and are the coarsest and the finest |
...
| `WHOSE_DEEPTAMSA_GOVERNS_IS_NOT_SAID` · `WHETHER_THE_ASPECT_NEEDS_THE_ORB_IS_NOT_SAID` · `THE_SMALLER_DEEPTAMSA_GOVERNS` | Ours: the rule left three holes and the example closed two — the orb is required and both must hold, so the smaller governs |
...
| `THAMBIRA_RULE` · `THAMBIRA_RESULTS` · `THAMBIRA_EXAMPLE` | §29.2.14 verbatim and its worked case as data |
...
| `FOOTNOTE_90` · `FOOTNOTE_90_CONTROVERSIES` | Footnote 90 verbatim, and its two controversies against the settings that already carry them |
| `THE_TWO_CONTROVERSIES_DWARF_THE_BORDER_THEY_ANNOTATE` | Ours: 5.8′ inside the Lahiri family, 139.8′ across six ayanamsas, 55′ of parallax, against a one-arcminute border. OI-182 |
