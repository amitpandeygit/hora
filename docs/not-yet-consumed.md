...
### Chapter 26 — §26.6, Tables 65 to 68 (16)

`transits/tara.py`. Tables 65 to 68 have arrived; only 69 is pending.
...
| `BODY_PART_TABLES` · `BODY_PART_TABLES_PENDING` · `TABLE_65_SUN` · `TABLE_66_MOON` · `TABLE_67_MARS` · `TABLE_68_MERCURY_JUPITER_VENUS` | The registry and the Sun's table. `body_part` and `grahas_dwelling_in` read them; the register entry stays while four tables are missing |
...
| `TWO_FEET_IS_THE_WIDEST_BLOCK_IN_EVERY_TABLE` | Ours, **narrowed** by Table 68: the six counts were a property of the single-graha tables, not of the rule |
| `THE_SHARED_TABLE_IS_THE_COARSER_ONE` | Ours: granularity tracks how many grahas share a table — eight rows and split hands against six rows and Two hands |
