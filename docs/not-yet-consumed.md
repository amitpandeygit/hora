...
| `SNAPSHOTS` · `SUNS_GOOD_HOUSES_ARE_THE_UPACHAYAS` · `SUN_AND_MOON_DIFFER_ONLY_ON_THE_1_7_AXIS` · `AGREEING_SNAPSHOTS_ARE_NOT_THE_SAME_RESULTS` · `THE_FIRST_THREE_TABLES_NEST` · `MARS_IN_THE_TENTH_IS_MARKED_BAD_AND_READS_GOOD` · `MERCURY_ALTERNATES_FOR_TEN_HOUSES` · `THE_11TH_AND_12TH_HAVE_NOT_VARIED` · `THE_TWELFTH_ROW_REPEATS_A_WORD` | The two verdicts the tables offer, and three findings about their structure that the book never states — it prints one table per graha and never compares them. `TABLE_53_SUN` and `TABLE_54_MOON` themselves **are** consumed, by `transit_result` and `agreement` |
