...
| **PVR-14** | Three amsa names | §6.6's table ("Uttamaamsa", "Devalokaamsa", "Brahmalokamsa") vs §11.7.2's prose ("Uttamsaamsa", "Devalokamsa", "Brahmalokaamsa") | Rule 2 — a definitional table outranks a passing mention. §6.6 is where the amsas are defined and is internally consistent across all four groups; §11.7.2 uses them in passing while grading a yoga. §6.6 already records the same kind of drift in its own closing sentence, which writes "Gopuramsa" for "Gopuraamsa" | §6.6's spellings; §11.7.2's are recorded as variants so either matches | `AMSA_SPELLINGS_IN_11_7_2` |
| **PVR-13** | Does a weakened yoga block a Sankhya yoga? | §11.5.4's rule ("apply if no other Naabhasa yogas mentioned previously are **applicable**") vs §11.5.4's own example, Lord Sri Rama's chart, given as Daama — whose only earlier Naabhasa yoga is a **Sarpa carrying §11.5.2's "may not operate well" clause** | Neither source is overruled. "Applicable" is read as excluding a yoga the book itself says does not fully operate, and rule and example then agree. The reading is forced by the example: on any other reading §11.5.4 contradicts itself on its own page | a weakened yoga does not block | [OI-80](open-items.md) |
| **PVR-12** | §11.4.5's yoga name | The heading "Hamsa Yoga" vs the Definition beneath it, "it is called **Ruchaka** yoga" — Ruchaka being Mars's yoga, defined in §11.4.1 | Rule 4 — corroboration. §11.4 promises "5 kinds of great persons", and two yogas sharing a name leaves four names for five. The heading, the results ("swan-like", and *hamsa* is Sanskrit for swan) and the element ordering all agree against the sentence | Hamsa | [D-30](book-deviations.md) |
| **PVR-11** | Adhi Yoga's houses | §11.3.6's rule ("the natural benefics occupy 6th, 7th and 8th from Moon") vs its own example (Moon in Taurus with benefics in Virgo and Leo, which are the 5th and 4th) | Rule 1 — the stated rule beats its transcribed output. The rule matches classical Adhi Yoga; the example matches nothing | the 6th, 7th and 8th | [D-28](book-deviations.md) |
...
### When PVR marks his own research

The book does this twice. §25.4 labels its whole section, and **footnote 86 of
§30.1** labels the rule that seeds two of chapter 30's three annual dasas by
progressing the natal chart: "This is a result of the author's own researches."
Everything below was written for §25.4 and applies to both.

§25.4 labels itself. "The actual
...
| elsewhere in the book | §25.4 and §30.1's footnote 86 |
|---|---|
...
constants and functions as §25.4's or §30.1's so that a later reader can tell which parts
